...
async def rag_node(state: AgentState, config: RunnableConfig) -> dict:
    if _DEMO_MODE:
        return {
            "rag_context": [],
            "response": (
                "Demo mode: This is a pre-scripted response. "
                "In production, this answer is generated by the AI using your clinical knowledge base. "
                "Run 'make start' instead of 'make demo' to enable real AI responses."
            ),
            "confidence": 0.9,
        }
...
async def reasoning_node(state: AgentState, config: RunnableConfig) -> dict:
    if _DEMO_MODE:
        return {
            "response": (
                "Demo mode: This is a pre-scripted clinical reasoning response. "
                "In production, the AI performs multi-step chain-of-thought reasoning over your query. "
                "Run 'make start' instead of 'make demo' to enable real AI reasoning."
            ),
            "chain_of_thought": "Demo mode — reasoning skipped.",
            "confidence": 0.85,
        }
